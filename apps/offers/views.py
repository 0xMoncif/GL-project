...
class StudentOfferListView(ListAPIView):  # this is for student (handle the case of cibled and uncibled offers)
    ...
    def get_queryset(self):
        ...
        return ( Offer.objects.filter(is_active=True).filter(Q(institutions__isnull=True) | Q(institutions=student.institution))
...
class StudentOfferDetailView(RetrieveAPIView):
    ...
    def get_object(self):
        ...
        if offer.institutions.exists() and student.institution not in offer.institutions.all():
...
class OfferListCreateAPIView(ListCreateAPIView):  
    ...
    def get_queryset(self):
        return Offer.objects.filter(company=self.request.user.companyProfile) # to return only company's own offers 
    
    def perform_create(self, serializer):   # this is to assign the user (company) to the offer created automatically 
        company_profile = self.request.user.companyProfile
        serializer.save(company=company_profile)


class CompanyOfferDetailView(RetrieveUpdateDestroyAPIView):  # this is for a single offer instance 
    serializer_class = OfferSerializer
